        ...
        public void Sauvegarde(string sourcePATH, string destPATH, bool copyDirs, int getStateIndex, long fileCount, int getIndex, string getName)
        {
            ...
            foreach (var file in files)
            {
                if (extensions.Contains(file.Extension))
                {
                    ...
                    var stateList1 = JsonConvert.DeserializeObject<List<Etat>>(jsonState) ?? new List<Etat>(); //convert a string into an object for JSON

                    stateList1[getStateIndex].TimeToCrypt = TimeToCrypt.ToString();

                    string ResultJsonState = JsonConvert.SerializeObject(stateList1, Formatting.Indented);  //convert an object into a string for JSON
